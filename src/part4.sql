...
BEGIN
    -- Общая часть запроса
    RETURN QUERY (WITH part1 AS (SELECT c.customer_id,
                                        c.group_id,
                                        gs.group_name,
                                        c.group_margin
                       ...
                       Required_Check AS (SELECT cards.customer_id,
                                          ...
                                          WHERE (calc_method = 1 AND t.transaction_datetime BETWEEN start_date AND end_date)
                                             OR (calc_method = 2 AND t.transaction_id >=
                                                                     (SELECT MAX(transaction_id) - transaction_count FROM transactions))
